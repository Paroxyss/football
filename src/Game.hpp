...
class Game {
  public:
    ...
    // Applique les accélérations liées aux forces de friction pour un temps
    // donné
    ...
    collisionList *
    get_object_collision_list(int objId,
                              collisionList *listToAppend = NULL);

    // Regarde si la balle est dans la cage {id}, ou alors qu'elle va
    // traverser la ligne dans la seconde suivante
    bool check_goal(int id);

    // Déplace tous les objets pour la durée donnée (en s)
    void move_all_obj(double time);

    // Écrit l'état des joueurs dans le fichier csv (si la sauvegarde est
    // activée)
    void write_players();
